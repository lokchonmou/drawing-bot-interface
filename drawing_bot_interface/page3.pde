class settingPage{
    ...
    void setup(){
        ...
        halftoneMethod = int(setting[5]);

        int buttonWidth = 80, buttonHeight = 50, tSize=18;
        penUpPlus = new Button(int((float)width*.5), int((float)height*.05), buttonWidth, buttonHeight, "+", tSize);
        penDownPlus = new Button(int((float)width*.5), int((float)height*.15), buttonWidth, buttonHeight, "+", tSize);
        feedRatePlus = new Button(int((float)width*.5), int((float)height*.25), buttonWidth, buttonHeight, "+", tSize);

        penUpMinus = new Button(int((float)width*.7), int((float)height*.05), buttonWidth, buttonHeight, "-", tSize);
        penDownMinus = new Button(int((float)width*.7), int((float)height*.15), buttonWidth, buttonHeight, "-", tSize);
        feedRateMinus = new Button(int((float)width*.7), int((float)height*.25), buttonWidth, buttonHeight, "-", tSize);

        setZeroButton = new Button(int((float)width*.8), int((float)height*.4), buttonWidth, buttonHeight, "SET\nZERO", 14);
        goZeroButton = new Button(int((float)width*.8), int((float)height*.5), buttonWidth, buttonHeight, "GO\nZERO", 14);
        backButton = new Button(int((float)width*.9), int((float)height*.9), buttonWidth, buttonHeight, "BACK", tSize);

        crossline = new Button(int((float)width*.25), int((float)height*.9), buttonWidth, buttonHeight, "crossline\nhalftone", 14);
        AM        = new Button(int((float)width*.45), int((float)height*.9), buttonWidth, buttonHeight, "AM\nhalftone", 14);
        
        buttonWidth = 50; tSize = 12;
        invertXButton = new Button(int((float)width*.3), int((float)height*.4), buttonWidth, buttonHeight, "", tSize, true);
        invertYButton = new Button(int((float)width*.3), int((float)height*.5), buttonWidth, buttonHeight, "", tSize, true);

        LEFTButton = new Button(int((float)width*.4), int((float)height*.7), buttonWidth, buttonHeight, "LEFT", tSize);
        RIGHTButton = new Button(int((float)width*.6), int((float)height*.7), buttonWidth, buttonHeight, "RIGHT", tSize);
        UPButton = new Button(int((float)width*.5), int((float)height*.6), buttonWidth, buttonHeight, "UP", tSize);
        DOWNButton  = new Button(int((float)width*.5), int((float)height*.7), buttonWidth, buttonHeight, "DOWN", tSize);
    }
    ...
    void draw(){
        ...
        fill(0);
        text("pen up\n"+penUp, int((float)width*.3), (float)height*.05);
        text("pen down\n"+penDown, int((float)width*.3), (float)height*.15);
        text("feed rate\n"+feedRate, int((float)width*.3), (float)height*.25);
        text("Invert X axis", int((float)width*.45), (float)height*.4);
        text("Invert Y axis", int((float)width*.45), (float)height*.5);
        String methodName="";    
        switch(halftoneMethod){case 0: methodName="Cross Line";break; case 1: methodName="AM";break;}
        text("Halftone Method:\n"+methodName, int((float)width*.5), (float)height*.8);
    ...
    public void mousePressed() {
        ...
        if(backButton.mousePressed()){conf = createWriter("configuration.txt");conf.println(penUp);conf.println(penDown);conf.println(feedRate);conf.println(invert_x);conf.println(invert_y);conf.println(halftoneMethod);conf.flush();conf.close();pages = 0;if(debug)println("SAVED");}
        ...
        if(crossline.mousePressed()) halftoneMethod=0;
        if(AM.mousePressed()) halftoneMethod=1;
    }
