...
boolean debug = true;
boolean portDebug = false;
int reSizeWidth = 984;     //100dpi=3.937dpmm, 250mm*3.937dpmm= reSizeWidthpixel
...
void setup() {
    ...
    p0.setup();
    p3.setting = loadStrings("configuration.txt");
    p3.penUp = int(p3.setting[0]);
    p3.penDown = int(p3.setting[1]);
    p3.feedRate = int(p3.setting[2]);
    p3.invert_x = boolean(p3.setting[3]);
    p3.invert_y = boolean(p3.setting[4]);

    if (!portDebug)    myPort.clear();
...
void FileSelected(File selection) {
    ...
    if (selection == null) {
        ...
        || s.subSequence(s.length()-4, s.length()).equals(".PNG")){
            ...
            surface.setSize(reSizeWidth, int(reSizeWidth*p5.photoRatio)); 
...
void camSelect(){
    ...
    surface.setResizable(true);
    surface.setSize(reSizeWidth, int(reSizeWidth*p4.photoRatio));  //100dpi=3.937dpmm, 250mm*3.937dpmm= reSizeWidthpixel
